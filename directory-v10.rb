...
# tries to load students given in file argument
...
def print_menu
	puts "\nMENU"
	puts "1. If required, load the existing student directory from students.csv"
	puts "2. Input the names of students to add into the directory"
	puts "3. Display student directory"
	puts "4. Permanently order the students by cohort"
	puts "5. Show students whose names begin with the letter 'A'"
	puts "6. Show students who have less than 12 letters in their name"
	puts "7. Save the file to students.csv"
	puts "8. Exit"
end
...
def process(selection)
	case selection
		when "1"
			load_students
		when "2" #carries out input_students method
			input_students			
			# input the students
		when "3"
			show_students
		...
		when "8"
			...
			puts "Press RETURN to return to the menu"
			STDIN.gets
	end
